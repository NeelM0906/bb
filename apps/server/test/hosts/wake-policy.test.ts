...
it("wakes for admission reservation and permits cleanup during removal without waking", async () =>
  withTestHarness(async (harness) => {
    const { host } = seedHostSession(harness.deps, { id: "admission-wake" });
    const resume = vi.fn(async () => ({ resource: { id: "restored" } }));
    installMachineProvider({
      resume,
      suspend: async () => ({ resource: { id: "saved" } }),
    ...
    const request = vi
      ...
      .mockResolvedValue({
        ...
        ok: true,
        result: { outcome: "reserved", reservation },
      });
    await expect(
      callHostOnlineRpcForWork(harness.deps, {
        hostId: host.id,
    ...
    request.mockResolvedValue({
      type: "host-rpc.response",
      requestId: "release",
      commandType: "host.admission.release",
      ok: true,
