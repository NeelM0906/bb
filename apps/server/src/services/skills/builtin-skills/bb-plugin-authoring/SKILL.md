...
Local path installs must live outside BB-managed personal workspaces and
worktrees because archiving their owning environment deletes those directories.
Move finished plugin sources to a stable checkout before installing them. The
`--allow-managed-workspace-source` flag is a destructive-lifecycle override,
