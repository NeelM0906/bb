...
export function settleThreadStorageDeleteCommandResult(
  ...
  return {
    postCommitActions: [
      {
        run: async (deps) => {
          await releaseDeletedThreadAdmission(deps, args.command.threadId);
        },
      },
    ],
  };
...
export function requestThreadStopForCurrentState(
  ...
  if (!isWaitingForAdmission && hasLiveThreadRuntime(deps, thread)) {
    if (environment === null) {
      return;
    }
...
async function releaseDeletedThreadAdmission(
  deps: LoggedPendingInteractionWorkSessionDeps,
  threadId: string,
): Promise<void> {
  await releaseThreadWorkAdmission(deps, {
    terminalReason: "thread deleted",
    threadId,
  });
  if (getCurrentThreadWorkAdmission(deps.db, threadId) === null) {
    finalizeStoppedThread(deps, { threadId, runtimeStopped: true });
  }
}
...
export function finalizeStoppedThread(
  ...
  if (finalized) {
    ...
    if (
      ...
      getCurrentThreadWorkAdmission(deps.db, args.threadId) !== null
    ) {
      void releaseDeletedThreadAdmission(deps, args.threadId).catch((error) => {
        deps.logger.warn(
          { err: error, threadId: args.threadId },
          "Failed to release deleted thread admission",
        );
      });
    }
    requestQueuedMessageDispatch(deps, {
...
export function finalizeStoppedThreadInTransaction(
  ...
  if (finalizedThread.deletedAt !== null) {
    ...
    if (providerEnvironmentHasPendingWork(deps.db, finalizedThread.id))
      return true;
    if (getCurrentThreadWorkAdmission(deps.db, finalizedThread.id) !== null)
      return true;
