...
it("removes ignored repositories from the batched cache and typed CLI", async () => {
  fake.responses.set("one", response());
  fake.responses.set("two", response());
  const { harness, bb } = await start("acme/one,acme/two");
  await harness.behavior.callRpc("refresh");
  await harness.behavior.setSettings({ ignoredRepos: "ACME/ONE" });
  await harness.behavior.callRpc("refresh");
  expect(
    bb.storage.database().prepare("SELECT DISTINCT repo FROM items").all(),
  ).toEqual([{ repo: "acme/two" }]);
  const result = await harness.runCli(["issues", "--json"]);
  expect(result.exitCode).toBe(0);
  expect(JSON.parse(result.stdout).items).toEqual([
    expect.objectContaining({ repo: "acme/two", number: 1 }),
  ]);
});
