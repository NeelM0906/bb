...
export default async function plugin(bb: BbPluginApi) {
  ...
  function patchCachedItem(
    ...
    if (patch.state !== undefined) {
      db.prepare(
        "UPDATE items SET state = ? WHERE LOWER(repo) = ? AND kind = 'issue' AND number = ?",
      ).run(patch.state, repoKey(repo), number);
    }
    if (patch.assignees !== undefined) {
      db.prepare(
        "UPDATE items SET assignees = ? WHERE LOWER(repo) = ? AND kind = 'issue' AND number = ?",
      ).run(JSON.stringify(patch.assignees), repoKey(repo), number);
    }
    if (patch.labels !== undefined) {
      db.prepare(
        "UPDATE items SET labels = ? WHERE LOWER(repo) = ? AND kind = 'issue' AND number = ?",
      ).run(JSON.stringify(patch.labels), repoKey(repo), number);
    }
      ...
      run(input) {
        return guarded(async () => {
          ...
          const items = listCachedItems({
            ...
            ...(repo === undefined ? {} : { repo }),
            state: "open",
            trackedRepos: (await discoverRepos()).map((entry) => entry.repo),
          });
  ...
  bb.cli.register(
    defineCli({
      ...
      description:
        "Tracked repositories come from extraRepos and optional writable project origin remotes, excluding ignoredRepos.",
      commands: {
        repos: cliCommand({
            ...
            return guarded(async () => {
              ...
              if (input.options.json) {
                return {
                  ...
                  stdout: JSON.stringify({
                    ok: true,
                    repos,
                    ignoredExtraRepos: invalidExtraRepos,
                  }),
                  ...warning,
                };
              }
              if (repos.length === 0) {
                return {
                  exitCode: 0,
                  stdout:
                    "No tracked repos. Set extraRepos or enable Follow BB project remotes for origins you can write to.",
                  ...warning,
                };
              }
              return {
