...
`,
  ...
  describe("ACP stale steering", () => {
    function createAcpSteerRuntime(mode: "stale" | "error") {
      const providerScriptPath = join(tmpDir, `acp-${mode}-steer-provider.cjs`);
      writeFileSync(
        providerScriptPath,
        `const readline = require("node:readline");
...
readline.createInterface({ input: process.stdin }).on("line", (line) => {
  ...
  if (message.method === "initialize") {
    ...
    send({ jsonrpc: "2.0", id: message.id, result: { providerThreadId: "provider-acp" } });
    send({ jsonrpc: "2.0", method: "thread/identity", params: {
      threadId: params.threadId, providerThreadId: "provider-acp"
    } });
  } else if (message.method === "turn/start") {
    send({ jsonrpc: "2.0", id: message.id, result: {} });
    send({ jsonrpc: "2.0", method: "turn/started", params: {
      threadId: params.threadId, providerThreadId: "provider-acp", turnId: "turn-1"
    } });
  } else if (message.method === "turn/steer") {
    if (mode === "stale") {
      send({ jsonrpc: "2.0", id: message.id, result: {
        status: "stale", activeTurnId: null
      } });
    } else {
      send({ jsonrpc: "2.0", id: message.id, error: {
        code: -32001, message: "ACP authentication expired"
      } });
    }
  }
});`,
      );
      const adapter = createFakeAdapter(providerScriptPath);
      return createAgentRuntimeWithAdapters({
        ...
        adapterFactory: () => ({ ...adapter, id: "acp-custom" }),
      });
    }

    async function startAcpTurn(
      runtime: ReturnType<typeof createAgentRuntimeWithAdapters>,
    ): Promise<void> {
      await runtime.startThread({
        environmentId: "env-1",
        threadId: "t1",
      ...
      await expect(
        runtime.waitForActiveTurn("t1", { timeoutMs: 1_000 }),
      ).resolves.toBe("turn-1");
    }

    it("returns stale and clears only the matching turn for an ACP stale result", async () => {
      const runtime = createAcpSteerRuntime("stale");
      await startAcpTurn(runtime);
      ...
      await expect(
        runtime.steerTurn({
          ...
          threadId: "t1",
          expectedTurnId: "turn-1",
          input: [promptTextInput({ text: "late follow-up" })],
          options: fullRuntimeOptions,
        }),
    ...
    it("keeps unrelated ACP steer errors as errors and preserves the active turn", async () => {
      const runtime = createAcpSteerRuntime("error");
      await startAcpTurn(runtime);
      ...
      ).rejects.toThrow("ACP authentication expired");
      expect(runtime.getActiveTurnId("t1")).toBe("turn-1");
