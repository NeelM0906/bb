...
Desktop browser control is available only while the desktop window uses its built-in local server. Custom and Connect servers cannot access desktop browser profiles. Cookie import copies signed-in sessions from a browser installed on the desktop host into a BB browser profile after explicit approval in a native desktop dialog naming the source and destination. Each import requires a new approval; switching servers or navigating the window invalidates pending approval. These two commands take `--host`, `--instance`, and `--generation` but no `--thread`:
