...
The builtin Provider retry plugin continues Codex and Claude Code turns after a
structured subscription window resets. It is enabled on fresh installations;
disable it under Extensions → Plugins or run `bb plugin disable provider-retry`.
It keeps its timers in memory, coordinates waits by machine/provider
