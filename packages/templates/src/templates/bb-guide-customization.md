...
or backend APIs.
The default-on `editMessages` experiment enables editing eligible, successfully
completed root user messages in idle Codex, Claude Code, and Pi threads; turn
it off to hide the editor. Opening the editor is client-local; submitting
replaces the selected turn and all later conversation history while retaining
workspace side effects. Grouped multi-message requests are not yet editable.
