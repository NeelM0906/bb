...
describe("events", () => {
  ...
  it.each(["turn/input/accepted", "client/turn/rejected"] as const)(
    "bounds large %s request lookups and preserves global sequence order",
    (type) => {
      const { db, thread } = setup();
      const requestIds = Array.from({ length: 1_005 }, (_, index) =>
        encodeClientTurnRequestIdNumber({ value: index + 1 }),
      );
      insertEvents(
        db,
        noopNotifier,
        requestIds.map((requestId, index) => ({
          ...
          type,
          ...(type === "turn/input/accepted"
            ? createTurnEventFields({ turnId: `turn-${index}` })
            : threadEventFields),
          data: JSON.stringify(
            type === "turn/input/accepted"
      ...
      const lookup =
        type === "turn/input/accepted"
          ? listStoredTurnInputAcceptedRowsByClientRequestIds
          : listStoredTurnRejectedRowsByClientRequestIds;
      const rows = lookup(db, {
        threadId: thread.id,
        afterSequence: 2,
        clientRequestIds: [...requestIds].reverse().concat(requestIds[0]),
      });
      expect(rows.map((row) => row.sequence)).toEqual(
        Array.from({ length: 1_003 }, (_, index) => index + 3),
      );
    },
  );
