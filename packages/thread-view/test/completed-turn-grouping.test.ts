...
describe("groupCompletedTurnMessages", () => {
  ...
  it("keeps narration that precedes work visible and keeps the response that precedes the terminal text", () => {
    ...
    expect(groups.summaryItems).toMatchObject([
      { kind: "ungrouped-message", message: { id: "narration" } },
      {
        kind: "summary",
  ...
  it("keeps every response in a run of adjacent assistant texts", () => {
    ...
    expect(groups.summaryItems).toMatchObject([
      ...
      { kind: "ungrouped-message", message: { id: "second" } },
      {
        kind: "summary",
        sourceMessages: [{ id: "command" }],
      },
